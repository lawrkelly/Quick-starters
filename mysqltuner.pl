...
sub mysql_stats {
	...
	if (!mysql_version_ge(4)) {
		...
		badprint "Query cache is disabled. In certain usage cases you may want to leave Query cache disabled i.e. InnoDB table usage\n";
	...
	if ($mystat{'Created_tmp_tables'} > 0) {
		if ($mycalc{'pct_temp_disk'} > 25 && $mycalc{'max_tmp_table_size'} < 256*1024*1024) {
			...
			push(@adjvars,"tmp_table_size (> ".hr_bytes_rnd($myvar{'tmp_table_size'}).", increase tmp_table_size)");
			push(@adjvars,"max_heap_table_size (> ".hr_bytes_rnd($myvar{'max_heap_table_size'}).", increase max_heap_table_size)");
	...
	if ($mystat{'Open_tables'} > 0) {
		if ($mycalc{'table_cache_hit_rate'} < 20) {
			...
			if (mysql_version_ge(5, 1)) {
				push(@adjvars,"table_cache (> ".$myvar{'table_open_cache'}.", table_open_cache hit rate <20%)");
			} else {
				push(@adjvars,"table_cache (> ".$myvar{'table_cache'}.", table_cache hit rate <20%)");
			}
...
sub make_recommendations {
	...
	if (@adjvars > 0) {
		...
		if ($mycalc{'pct_physical_memory'} > 90) {
			print "  *** MySQL maximum memory usage is dangerously high ***\n  *** Add RAM before increasing MySQL buffer variables ***\n".
